...
public class PobedaService {
	...
	private static final List<String> BAD_CITIES = Arrays.asList(new String[]{"VKO", "","{}", "TJM", "PEE", "KUF", "SCW", "KVX", "EGO", "NJC", "CSY", "CEK", "UFA", "SGC", "IGT"});
    private static final Logger logger = Logger.getLogger(PobedaService.class.getName());
	...
	public Map<String, Map<String, Integer>> invokeAll() throws Exception {
        ...
        for (String city : cities) {
            if (BAD_CITIES.contains(city)) continue;
    ...
    public Map<String, Integer> invoke(String from, String to, int mon){
        try {
            return invoke(buildCriterion(from, to, getDateForMonth(mon, false), getDateForMonth(mon, true)));
        } catch (Exception e) {
    ...
    public Map<String, Integer> invoke(SearchCriterion searchCriterion) throws Exception {
		logger.info(searchCriterion.getFrom() + " > " + searchCriterion.getTo() + " " + searchCriterion.getOutboundDate() + " " + searchCriterion.getInboundDate());
        String searchUrl = String.format(SEARCH_URL, searchCriterion.getTripType(), searchCriterion.getFrom(), searchCriterion.getTo(), searchCriterion.getInboundDate(), searchCriterion.getOutboundDate());
        logger.info(searchUrl);
		Map<String, String> cookies = Jsoup.connect(searchUrl).data("t", new Date().getTime()+"")
				.userAgent(
		...
		String safeString = "_ga=1; _ym_uid=1; ASP.NET_SessionId=" + sessionId + "; skysales=" + skysales + "; _ym_isad=1;";
		safeString += URLEncoder.encode(searchCriterion.toJson(), "UTF-8");
        logger.info(cookies.toString());
		Map<String, Integer> rsMap = fetch(safeString, searchCriterion.getOutboundDate(), "VKO".equals(searchCriterion.getTo())? searchCriterion.getFrom() : searchCriterion.getTo());
		return rsMap;
	...
	private static Map<String, Integer> fetch(String searchCriterion, String dateSelected, String toCity)
			throws Exception {
		Map<String, Integer> results = new HashMap<String, Integer>();
		Connection.Response res = Jsoup
				.connect(AVL_MONTH_URL)
				.data("dateSelected", dateSelected, "indexTrip", new Date().getTime()+"")
				.userAgent(
						"Mozilla/5.0 (Linux; U; Android 4.0.3; ko-kr; LG-L160L Build/IML74K) AppleWebkit/534.30 (KHTML, like Gecko) Version/4.0 Mobile Safari/534.30")
		...
		for (Element el : els) {
            ...
            if (THRESHOLDS.get(toCity) !=null && THRESHOLDS.get(toCity) <= price) continue;
    ...
    public static void email(Map<String, Map<String, Integer>> map){
        ...
        try {
            ...
            msg.setFrom(new InternetAddress("mailer@pobeda-aero.appspotmail.com", "pobeda-aero mailer"));
            msg.addRecipient(Message.RecipientType.TO,
                    new InternetAddress("barthezzko@gmail.com", "Mikhail Baytsurov"));
